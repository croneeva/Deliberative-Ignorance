---
title: "Deliberative Ignorance 2"
author: "Eva Crone"
date: "2025-10-21"
...
data_wide <- dat_short %>%
  select(ResponseId, Age_group, Scenario_Code, all_of(motive_vars)) %>%
  # long -> wide
  pivot_wider(
    id_cols = c(ResponseId, Age_group), 
...
## 3.4. Number of ratings by scenario and age group
```{r}
dat_short <- dat_short %>%
...
ggplot(di_scenario, aes(x = Scenario_Content, y = n_ratings, fill = factor(want_to_know_bin))) +
  ...
  scale_fill_manual(values = c("0" = "skyblue", "1" = "darkorange"),
                    name = "Want to know?",
...
## 6.4 Top 3 highest average rated motives by category
```{r}
...
ratings_long_DI <- ratings_per_motive_DI %>%
  ...
  mutate(
    ...
    want_to_know_bin = factor(
      want_to_know_bin,
      levels = c(0, 1),
      labels = c("Want to know", "Not want to know")
    )
  )
...
ggplot(ratings_long_DI, aes(x = motive, y = mean_rating, group = want_to_know_bin, color = want_to_know_bin)) +
  ...
  theme_minimal(base_size = 12) +
  labs(
    title = "Average Motive Ratings by 'Want to Know' vs. 'Not Want to Know'",
    x = "Motive",
    y = "Mean Rating",
    color = "Response"
  ) +
